...
Private registration codes share the account's `otpCodeSeparate` setting with
sign-in and password recovery. `otpRegistrationExplanation` configures the
explanation sent as a quoted reply to an isolated registration code. Leave it
blank for the Portuguese or English default. Configure the account's Global
scope when present; otherwise every enabled Connector scope must agree on these
private-message settings. Code and explanation use WABP's sensitive delivery
service and are excluded from captured history, echoes, replay, and quotations.
